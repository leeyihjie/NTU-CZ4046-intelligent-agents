...
vresults_path = os.path.join(path, 'part_1', 'results', 'Value Iteration')
presults_path = os.path.join(path, 'part_1', 'results', 'Policy Iteration')
...
try:
    ...
    pass
rfile_vi = os.path.join(vresults_path, 'VI vs Utility [Part 1 Normal Maze].png')
rfile_pi = os.path.join(presults_path, 'PI vs Utility [Part 1 Normal Maze].png')
...
Value Iteration (Part 1).csv
Policy Iteration (Part 1).csv
"""

value_df = pd.read_csv(os.path.join(vresults_path, 'Value Iteration (Part 1).csv'), names = value_headers, index_col='State')
policy_df = pd.read_csv(os.path.join(presults_path, 'Policy Iteration (Part 1).csv'), names = policy_headers, index_col='State')
...
plt.title("VI vs Utility [Part 1: Normal Maze]")
...
plt.title("PI vs Utility [Part 1: Normal Maze]")
plt.xlabel("Iterations")
plt.ylabel("Utility Values")
plt.legend(loc=2, prop={'size': 6})
